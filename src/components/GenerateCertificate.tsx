...
export default function GenerateCertificate({
  ...
  const handleDownload = async () => {
    ...
    link.href = imgData;
    link.download = `${certificateName}-certificate.png`;
    link.click();
  };
  ...
  const generateCertificateImage = useCallback(async () => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `certificates/${certificateName}-certificate.png`
      );
      await uploadString(storageRef, imgData, "data_url");

      // Get the public URL of the uploaded image
      const downloadURL = await getDownloadURL(storageRef);
      console.log("Certificate URL:", downloadURL);
  ...
  const shareUrl = `Check out my certificate! Here it is: ${encodeURIComponent(imageUrl)}`;
  const whatsappUrl = `https://api.whatsapp.com/send?text=${shareUrl}`;

  return (
    <div className="flex flex-col items-center justify-center w-full">
      {/* Certificate Preview */}
      <div className="relative bg-[#262930] p-4 rounded-xl shadow-lg border border-[#333842] mb-6 h-[250px] md:h-[315px] lg:h-[530px] w-[330px] md:w-[430px] lg:w-[730px]">
        <div className="inline-block">
          <div
            className="relative rounded-lg  scale-75 md:scale-100 lg:scale-[175%] origin-top-left w-[396px]"
            id="certificate"
          >
            <ExportedImage
              src={certificate}
            ...
            <h2 className="absolute top-[130px] left-[135px] font-serif">
              {certificateName}
            </h2>
          </div>
        </div>
      ...
      <div className="mt-6 text-center bg-[#262930] p-4 rounded-lg border border-[#333842] w-full max-w-md">
        <p className="text-gray-300 text-sm">
          Your certificate has been automatically downloaded. You can download it
